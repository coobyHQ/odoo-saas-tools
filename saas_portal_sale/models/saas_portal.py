...
class SaasPortalPlan(models.Model):
    ...
    def create_new_database(self, **kwargs):
        ...
        max_users = int(self.max_users) # or ir_params.sudo().get_param('saas_client.max_users')
        total_storage_limit = int(self.total_storage_limit) # or ir_params.sudo().get_param('saas_client.total_storage_limit')
        ...
        if order_id and client_obj and self.topup_ids:
            ...
            for topup in self.topup_ids:
                ...
                #order_lines = self.env['sale.order.line'].sudo().search([('order_id', '=', int(order_id)), ('product_id', '=', topup.product_tmpl_id.id)])
                if order_lines:
